...
variable "cloud_logs_resource_keys" {
  description = "The definition of the resource keys to generate. [Learn more](https://github.com/terraform-ibm-modules/terraform-ibm-obersevability-da/tree/main/solutions/instance/DA-types.md#resource-keys)."
  type = list(object({
    name                      = string
...
variable "cloud_logs_service_credential_secrets" {
  type = list(object({
  ...
  default     = []
  description = "Service credential secrets configuration for Observability. [Learn more](https://github.com/terraform-ibm-modules/terraform-ibm-observability-da/tree/main/solutions/instance/DA-types.md#service-credential-secrets)."

  validation {
    # Service roles CRNs can be found at https://cloud.ibm.com/iam/roles, select Cloud Logs / Cloud Monitoring and select the role
    condition = alltrue([
      for group in var.cloud_logs_service_credential_secrets : alltrue([
...
variable "cloud_monitoring_service_credential_secrets" {
  type = list(object({
    secret_group_name        = string
    secret_group_description = optional(string)
    existing_secret_group    = optional(bool)
  ...
  validation {
    # Service roles CRNs can be found at https://cloud.ibm.com/iam/roles, select Cloud Logs / Cloud Monitoring and select the role
    condition = alltrue([
      for group in var.cloud_monitoring_service_credential_secrets : alltrue([
        # crn:v?:bluemix; two non-empty segments; three possibly empty segments; :serviceRole or role: non-empty segment
        for credential in group.service_credentials : can(regex("^crn:v[0-9]:bluemix(:..*){2}(:.*){3}:(serviceRole|role):..*$", credential.service_credentials_source_service_role_crn))
      ])
    ])
...
variable "skip_secrets_manager_auth_policy" {
  type        = bool
  default     = false
  description = "Whether an IAM authorization policy is created for Secrets Manager instance to create a service credential secrets for Cloud Logs or Cloud Monitoring. Set to `true` to use an existing policy."
}
